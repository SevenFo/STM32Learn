...
def generateCPJson(armDir="C:\\Program Files (x86)\\GNU Tools Arm Embedded\\9 2019-q4-major\\",STMModle="STM32F429xx"):
    ...
    return json.dumps(c_cpp_properties)
def generateLaucnJson(configFiles = ["stlink.cfg","stm32f4x.cfg"],svdFile = "STM32F429x.svd",projectName = conf.projectName):
    lauch = {
...
def stmconfChanger():
    ...
    print("changed")
def generateJson():
    if(os.path.exists(conf.projectDir+".vscode")):
        print(".vscode existed")
    else:
        print("mkdir .vscode")
        os.mkdir(conf.projectDir+".vscode")
    with open(".vscode\\c_cpp_properties.json","w+") as f:
        f.write(generateCPJson())
    with open(".vscode\\launch.json","w+") as f:
        f.write(generateLaucnJson())
    with open(".vscode\\tasks.json","w+") as f:
        f.write(generateTaskJson())
    print("Json write successed")
